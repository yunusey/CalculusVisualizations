...
func _on_interface_disk_pos_changed(value: float) -> void:
	disk_pos = value
	update_rects($CanvasLayer/Interface.get_num_rect())

func update_rects(num_rects: int) -> void:
...
func color_rects_by_distinction() -> void:
	...
	for i in num_rects:
		...
		var color = distinction_color if i == disk_rect_index else ordinary_color
